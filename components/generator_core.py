...
class Agent(nn.Module):
    ...
    def __init__(self, hidden_size, with_attr=False, attr_size=-1, norm_type=None):
        ...
        if with_attr:
            self.attr_embedding = nn.Linear(attr_size, hidden_size).double()
        self.seed_embedding = nn.Linear(1, hidden_size, bias=False).double()
        self.node_embedding = nn.Linear(1, hidden_size, bias=False).double()
        self.input_mapping = nn.Sequential(
            make_linear_block(hidden_size, hidden_size, Swish, norm_type).double(),
            make_linear_block(hidden_size, hidden_size, Swish, norm_type).double()
        )
        self.pooling_layer = nn.Sequential(SelfAttnPooling(hidden_size), Swish())
        self.value_layer = nn.Linear(hidden_size, 1).double()
        self.node_score_layer = nn.Linear(hidden_size, 1, bias=False).double()
        self.stopping_score_layer = nn.Linear(hidden_size, 2, bias=False).double()
